...
class DimacsLoaderConfiguration{
    ...
    public int numberOfClausules = ALL_CLAUSULES;
...
public class BDDDimacsLoader {
     ...
     */
    public BDD loadFile(DimacsLoaderConfiguration config){
          ...
          while ((line = br.readLine()) != null && line.length()>0)
          {
              ...
              if (line.charAt(0)!='c')
              {
                  if(line.charAt(0)=='p')
                  {
                      ...
                      numVariables = Integer.parseInt(content[2]);
                      numClausules = Integer.parseInt(content[3]);
                      if(clausules != DimacsLoaderConfiguration.ALL_CLAUSULES && clausules>0 && clausules<numClausules)
